...
export function Category() {
    const {name,keyword,type}=useParams()
    ...
    const {data:movies_data,refetch,isFetching,isSuccess}=useQuery({
        queryKey:['movie_data'],
        queryFn:()=>{
            return discover_movies(contry?.value,year?.value,genre_selected?.value,keyword,firstPage,type)
        },
        refetchIntervalInBackground:false,
        refetchInterval:false,
                    ...
                    <div className={"mt-10 flex flex-wrap justify-center items-center gap-6 w-[90%] mx-auto"}>
                        {movies_data?.results?.map((item) => (
                            <Card img={item.poster_path} imdb={item.vote_average}
                                  date={type === "movie" || type === "الافلام" ? item.release_date : item.first_air_date}
                                  title={(type === "movie" || type === "الافلام") ? item.title : item.name}
                                  id={item.id}
                                  type={type}/>
